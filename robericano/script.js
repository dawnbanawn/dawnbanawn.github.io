...
function calculateMatchTime(message) {
  ...
  if (message == "matchTime" ) {
    return parseInt(gameTime) / matches - (parseInt(gameTime) / matches) * 0.0625;
  } else if (message == "breakTime" ) {
    return (parseInt(gameTime) / matches) * 0.0625;
  } else if (message == "matchTime00:00" ) {
    let tempTime = parseInt(gameTime) / matches - (parseInt(gameTime) / matches) * 0.0625;
    ...
    return Math.floor(
      (parseInt(gameTime) / matches - (parseInt(gameTime) / matches) * 0.0625) * 60
    );
  } else if (message == "breakInSeconds" ) {
    return Math.floor((parseInt(gameTime) / matches) * 0.0625 * 60);
  }
}
...
player03MatchCount = 0;
 ...
 numberOfPlayers = 0;
 playerNamesArray = [];
 timePerGame = 0; //gametime divided by matches.
//What the random match order is.
 chosenMatchOrder = 0;
 stopTimer();
 seconds = 0;
